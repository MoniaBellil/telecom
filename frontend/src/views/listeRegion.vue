...
<template>
  <vx-card title="Liste Region" code-toggler>
    <vs-button type="filled" @click.prevent="goToAdd" class="mt-5 block">Ajouter Region</vs-button>
    <vs-table :data="users">
      <template slot="thead">
        <vs-th>name</vs-th>
        <vs-th>Centrale</vs-th>
      ...
      <template slot-scope="{data}">
        <vs-tr :key="indextr" v-for="(tr, indextr) in data">
          ...
          <vs-td :data="tr.centrale_id">
            {{getName(tr.centrale_id)}}
          </vs-td>
...
export default {
  ...
  created() {
    axios.get('http://localhost:8000/api/getRegion')
  ...
  methods: {
    ...
    getName(id) {
      this.form.id=id;
      axios.post('http://localhost:8000/api/getCentraleById', this.form, {
